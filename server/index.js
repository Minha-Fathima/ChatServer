  ...
  const insertMessage = async (workspaceId, senderId, receiverId, room, message, messageType) => {
    try {
      const query = `
        INSERT INTO Messages (WsId, SenderId, ReceiverId, Room, Message, MessageType)
        VALUES (?, ?, ?, ?, ?, ?)
      `;
      const values = [workspaceId, senderId, receiverId, room, message, messageType];
      await queryAsync(query, values);
      console.log('Message successfully inserted into the database:', { workspaceId, senderId, receiverId, room, message, messageType });
      return true;
    } catch (error) {
...
async function handleTextMessage(data, type) {
  ...
  try {
    ...
    form.append('lang', 'en'); // Assuming English, modify as needed
    form.append('mode', 'rules');
    form.append('api_user', '1032749883');
    form.append('api_secret', '6cWqqCnVH3QBxNfB2ePLMEeDGSXpi7wU');
    ...
    const response = await axios({
      ...
      await insertMessage(workspaceId, senderId, receiverId, null, finalMessage, 'text');
...
async function handleFileMessage(data, type) {
  ...
  try {
    ...
    await insertMessage(workspaceId, senderId, receiverId, null, finalMessage, finalMessageType);
...
//   await insertMessage(workspaceId, senderId, receiverId, null, message, MessageType);
